...
```sql
-- GROUP BYや集計クエリ
SELECT JyoCD, COUNT(*) FROM NL_RA GROUP BY JyoCD;
-- SQLite: 150ms
-- DuckDB: 20ms (7.5倍高速)

-- JOIN + 集計
SELECT r.JyoCD, AVG(s.Time)
FROM NL_RA r JOIN NL_SE s ON ...
GROUP BY r.JyoCD;
-- SQLite: 800ms
-- DuckDB: 120ms (6.7倍高速)
```
...
-- テーブルをコピー
CREATE TABLE NL_RA AS SELECT * FROM sqlite_db.NL_RA;
CREATE TABLE NL_SE AS SELECT * FROM sqlite_db.NL_SE;
-- ... 他のテーブルも同様
```
...
[Test 1] Getting tables...
  Found 22 tables
[Test 2] Getting schema for NL_TK...
...
[Test 1] Getting tables...
  Found 22 tables
[Test 2] Getting schema for NL_RA...
  [PASS] All required columns present
